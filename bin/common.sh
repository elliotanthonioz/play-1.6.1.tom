...
download_play_official() {
  ...
  if [ -f "${BUILD_DIR}/${playZipFile}" ]; then
    echo "Using bundled ${playZipFile} from repository" | indent
    cp "${BUILD_DIR}/${playZipFile}" .
  else
    error "Play framework zip not found: ${BUILD_DIR}/${playZipFile}
Please ensure ${playZipFile} is committed to the repository root."
    exit 1
  fi
